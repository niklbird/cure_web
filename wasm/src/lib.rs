...
fn is_base64(s: &str) -> bool {
    let base64_regex = Regex::new(r"^(?:[A-Za-z0-9+/]{4}\s*)*(?:[A-Za-z0-9+/]{2}==\s*|[A-Za-z0-9+/]{3}=\s*)?$").unwrap();
...
pub fn test(){
    ...
    let mut state = State::load_example("tls").unwrap();
    state.get_nodes();
...
pub const EXAMPLE_CERT: &str = "MIIFazCCA1OgAwIBAgIRAIIQz7DSQONZRGPgu2OCiwAwDQYJKoZIhvcNAQELBQAw
